...
Content:
apply threshold on _luv_l_channel
*/
Mat Threshold::luv_thresh(int thresh_min, int thresh_max) {
	return threshold_process(_luv_l_channel, thresh_min, thresh_max, false);
}
...
Content:
apply threshold on _lab_b_channel
*/
Mat Threshold::lab_thresh(int thresh_min, int thresh_max) {
	return threshold_process(_lab_b_channel, thresh_min, thresh_max, false);
}
...
Mat Threshold::combine_thresh(Mat src) {
	...
	*/
	...
	if (_type & LUV_L) f.push(async(launch::async, &Threshold::luv_thresh, this, MIN_LUV_THRESH, MAX_LUV_THRESH));
	if (_type & LAB_B) f.push(async(launch::async, &Threshold::lab_thresh, this, MIN_LAB_THRESH, MAX_LAB_THRESH));
	//printf("Time taken: %.2fs\n", (double)(clock() - tStart) / CLOCKS_PER_SEC);
	if (_type & SOBEL_DIR) {
		printf("0\n");
	...
	if (_type & LUV_L) {
			printf("9\n");
		grad_luv = f.front().get();
		f.pop();
	}
	else grad_luv = Mat::zeros(Size(src.cols, src.rows), CV_8U);
	if (_type & LAB_B) {
			printf("10\n");
		grad_lab = f.front().get();
		f.pop();
	}
	else grad_lab = Mat::zeros(Size(src.cols, src.rows), CV_8U);
	//printf("Time taken: %.2fs\n", (double)(clock() - tStart) / CLOCKS_PER_SEC);
...
//	imshow("lap", grad_lap);
	...
	imshow("luv_l", grad_luv);
	imshow("lab_b", grad_lab);
...
*/	
	// 颜色融合 （单纯用颜色效果还可以）
	bitwise_or(grad_bgr, grad_luv, col);
	bitwise_or(col, grad_lab, dst);

	// 梯度和颜色按照权重融合
	//addWeighted(grad_bgr,0.8, grad_x, 0.2, -1, dst);
...
//	_type = SOBEL_X | BGR_R; //here to change way of combine
	_type = SOBEL_X | BGR_R | LUV_L | LAB_B;
	_type = _type | SOBEL_Y | HLS_S | CAN;
